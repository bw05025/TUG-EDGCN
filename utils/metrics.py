...
def jitter_score(pred, label, weight, bg_class=["background"]):
    ...
    py_correspond = np.linspace(-1,-y_segment,num=y_segment+1, dtype=int)
    # Correspondence vector "C" to record the index of corresponding segment in prediction for each ground truth segment
    ...
    margin = [] # vector "M": subtraction between each two adjacent entries in "C"
    for i in range(len(y_len)):
        margin.append(py_correspond[i + 1] - py_correspond[i])
    order_penalty = sum(np.maximum(0, (np.minimum(0, margin))))/(y_segment)

    pred_diff = []
    jitter_len = []

    n_cls_1 = y_label[0].item()  # "related action" 1
    n_cls_2 = y_label[0].item()  # "related action" 2

    for i in range(0,len(p_len)):
        if i in py_correspond[1:]:    # corresponded segments in the prediction
            seg = int(np.argwhere(py_correspond[1:] == i)[0])
            # update the "related action"
            n_cls_1 = y_label[seg].item()
            if seg == len(y_len)-1:
                n_cls_2 = y_label[seg].item()
            else:
                n_cls_2 = y_label[seg + 1].item()
        else:    # jittered segments in the prediction
    ...
    jitter_penalty = np.tanh(0.05 * np.sum(np.multiply(pred_diff, jitter_len)))   # μ=0.05 to control the penalization curve

    score = (1 - weight) * (1 + order_penalty) + weight * (1 - jitter_penalty)
